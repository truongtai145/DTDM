...
// ── PROMETHEUS METRICS ──────────────────────────────────────
// ══════════════════════════════════════════════════════════════
// Registry riêng cho app này (không dùng global register mặc định
// để tránh trùng metric khi hot-reload trong dev).
const metricsRegistry = new promClient.Registry();

// Thu thập sẵn các metric mặc định của Node.js process:
...
// http_requests_total{method,route,status} — đếm tổng số request
const httpRequestsTotal = new promClient.Counter({
  name: "http_requests_total",
  help: "Tổng số HTTP request đã nhận",
  labelNames: ["method", "route", "status"],
  registers: [metricsRegistry],
});

// response_time (giây) — histogram đo thời gian xử lý mỗi request
// Prometheus dùng histogram này để tính request_per_second qua
// rate(response_time_seconds_count[1m]) trong Grafana.
const responseTimeSeconds = new promClient.Histogram({
  name: "response_time_seconds",
  help: "Thời gian phản hồi mỗi HTTP request (giây)",
  labelNames: ["method", "route", "status"],
  buckets: [0.001, 0.005, 0.01, 0.05, 0.1, 0.3, 0.5, 1, 2, 5],
  registers: [metricsRegistry],
});

// redis_connection_status — 1 = đang kết nối, 0 = mất kết nối
const redisConnectionStatus = new promClient.Gauge({
  name: "redis_connection_status",
  help: "Trạng thái kết nối Redis (1=connected, 0=disconnected)",
  registers: [metricsRegistry],
});
redisConnectionStatus.set(0);

// current_pod_count — số Pod đang "alive" (heartbeat < 8s), lấy
// từ Redis nên mọi Pod đều báo cáo cùng một con số thống nhất.
const currentPodCount = new promClient.Gauge({
  name: "current_pod_count",
  help: "Số lượng Pod hiện đang hoạt động (heartbeat còn sống)",
  registers: [metricsRegistry],
});

// event_loop_delay_seconds — độ trễ event loop, cảnh báo khi
// Node.js bị nghẽn xử lý (CPU-bound hoặc quá tải).
const eventLoopDelaySeconds = new promClient.Gauge({
  name: "event_loop_delay_seconds",
  help: "Độ trễ trung bình của Node.js event loop (giây)",
  registers: [metricsRegistry],
});
const elDelayHistogram = monitorEventLoopDelay({ resolution: 20 });
elDelayHistogram.enable();
setInterval(() => {
  // .mean tính bằng nanosecond -> đổi ra giây
  eventLoopDelaySeconds.set(elDelayHistogram.mean / 1e9);
  elDelayHistogram.reset();
}, 5000);

// Cập nhật redis_connection_status theo sự kiện thật của client
store.on("connect", () => redisConnectionStatus.set(1));
store.on("ready", () => redisConnectionStatus.set(1));
store.on("end", () => redisConnectionStatus.set(0));
store.on("error", () => redisConnectionStatus.set(0));

// Cập nhật current_pod_count mỗi 5s bằng cách đọc heartbeat trong Redis
setInterval(async () => {
  try {
    const hb = (await store.hGetAll("pod_hb")) || {};
    const now = Date.now();
    const aliveCount = Object.values(hb).filter(
      (t) => now - parseInt(t) < 8000,
    ).length;
    currentPodCount.set(aliveCount);
  } catch {}
}, 5000);
...
.tab-btn{border:none;border-radius:999px;padding:10px 18px;font-weight:700;font-size:.85rem;cursor:pointer;transition:.2s;background:rgba(255,255,255,.1);color:#fff;margin-right:6px;}
.tab-btn.active{background:#ffffff;color:#1f2937;}
.tab-btn:hover{background:rgba(255,255,255,.2);}
.page-tabs{margin-top:18px;display:flex;flex-wrap:wrap;gap:8px;}
.page-view{display:none;}
.page-view.active{display:block;}
.card-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-bottom:16px;}
@media(max-width:900px){.card-grid{grid-template-columns:1fr;}}
.metric-card{background:var(--card);border:1px solid var(--border);border-radius:14px;padding:18px;min-height:120px;display:flex;flex-direction:column;justify-content:space-between;}
.metric-title{font-size:.8rem;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;}
.metric-value{font-size:2rem;font-weight:800;color:var(--blue);}
.metric-sub{font-size:.78rem;color:var(--muted);margin-top:6px;}
.chart-card{background:var(--card);border:1px solid var(--border);border-radius:14px;padding:18px;}
.chart-title{font-size:.9rem;font-weight:800;margin-bottom:12px;}
.chart-canvas{width:100%;height:320px;}
.stat-list{display:grid;gap:10px;}
.stat-item{background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:14px;display:flex;justify-content:space-between;align-items:center;}
...
<div class="header">
  <div class="header-inner">
    ...
    <div class="page-tabs" id="page-tabs">
      <button class="tab-btn active" data-page="demo" onclick="showPage('demo')">Demo</button>
      <button class="tab-btn" data-page="admin" onclick="showPage('admin')">Admin Dashboard</button>
      <button class="tab-btn" data-page="analytics" onclick="showPage('analytics')">Analytics</button>
    </div>
  </div>
</div>
<div class="main">
<div id="page-demo" class="page-view active">
<!-- ── SCENARIO BAR ── -->
...
<div id="page-admin" class="page-view">
  <div class="section">
    <div class="section-head">
      <div class="section-num" style="background:var(--blue-bg);color:var(--blue);">A</div>
      <div>
        <div class="section-title">Admin Dashboard</div>
        <div class="section-sub">Tổng quan cluster, Redis, HPA và trạng thái hiện tại</div>
      </div>
    </div>
    <div class="card-grid">
      <div class="metric-card">
        <div class="metric-title">Pods</div>
        <div class="metric-value" id="admin-pods">3</div>
        <div class="metric-sub">Số pod đang hoạt động</div>
      </div>
      <div class="metric-card">
        <div class="metric-title">CPU</div>
        <div class="metric-value" id="admin-cpu">8%</div>
        <div class="metric-sub">CPU sử dụng trung bình</div>
      </div>
      <div class="metric-card">
        <div class="metric-title">Memory</div>
        <div class="metric-value" id="admin-mem">120Mi</div>
        <div class="metric-sub">Memory ước lượng</div>
      </div>
      <div class="metric-card">
        <div class="metric-title">Requests</div>
        <div class="metric-value" id="admin-req">0</div>
        <div class="metric-sub">Tổng số request đã ghi</div>
      </div>
      <div class="metric-card">
        <div class="metric-title">Redis</div>
        <div class="metric-value" id="admin-redis">Connected</div>
        <div class="metric-sub">Trạng thái kết nối Redis</div>
      </div>
      <div class="metric-card">
        <div class="metric-title">HPA</div>
        <div class="metric-value" id="admin-hpa">IDLE</div>
        <div class="metric-sub">Trạng thái HPA hiện tại</div>
      </div>
    </div>
    <div class="section">
      <div class="section-head">
        <div class="section-num" style="background:var(--orange-bg);color:var(--orange);">S</div>
        <div>
          <div class="section-title">Cluster Status</div>
          <div class="section-sub">Tổng quan sức khỏe cluster và các pod</div>
        </div>
      </div>
      <div class="stat-list">
        <div class="stat-item"><strong>Service</strong><span id="admin-status">Healthy</span></div>
        <div class="stat-item"><strong>Redis Heartbeat</strong><span id="admin-redis-heartbeat">Live</span></div>
        <div class="stat-item"><strong>Pod Count</strong><span id="admin-pod-count">3</span></div>
        <div class="stat-item"><strong>Refresh</strong><span>cập nhật mỗi giây</span></div>
      </div>
    </div>
  </div>
</div>
...
const cpuHistory = Array(HISTORY_LENGTH).fill(8);
const memHistory = Array(HISTORY_LENGTH).fill(120);
...
function updateAdminMetrics() {
  ...
  document.getElementById('admin-pods').textContent = aliveCount;
  document.getElementById('admin-cpu').textContent = Math.round(hpaCpu) + '%';
  document.getElementById('admin-mem').textContent = Math.round(memHistory[(historyIndex-1+HISTORY_LENGTH)%HISTORY_LENGTH]) + 'Mi';
  document.getElementById('admin-req').textContent = totalRequests.toLocaleString();
  document.getElementById('admin-redis').textContent = redisStatus ? 'Connected' : 'Disconnected';
  document.getElementById('admin-hpa').textContent = hpaPhase.toUpperCase();
...
function renderCharts() {
  drawLineChart('chart-requests', requestHistory, '#2563eb', 'Requests / sec');
  drawLineChart('chart-cpu', cpuHistory, '#16a34a', 'CPU %');
  drawLineChart('chart-mem', memHistory, '#7c3aed', 'Memory Mi');
  drawLineChart('chart-pods', podHistory, '#ea580c', 'Pod Count');
}
...
const server = http.createServer(async (req, res) => {
  ...
  // ── Đo thời gian xử lý + đếm request cho MỌI route ──────────
  // route dùng để gộp nhãn (label) tránh cardinality quá lớn
  // (ví dụ /events không tính vì nó là kết nối stream dài hạn).
  const routeLabel = url.pathname === "/" ? "/dashboard" : url.pathname;
  const isStreamRoute = routeLabel === "/events";
  ...
  if (!isStreamRoute) {
    res.on("finish", () => {
      ...
      responseTimeSeconds.observe(labels, durationSec);
    });
  }
